% filepath: c:\Users\paolo\Desktop\AI_Agent\ChatBot_v5\ChatBot_thesis\papers\rag_scientific_paper.tex
\documentclass[conference]{IEEEtran}

% --- Essential Packages (IEEE manages layout; geometry removed) ---
...
\usepackage{caption}
\usepackage{placeins}
\usepackage{tabularx} % for auto column width control
\usepackage{array}    % extended column types
% Float spacing tuning for IEEEtran (conservative)
\setlength{\textfloatsep}{6pt plus 2pt minus 2pt}
\setlength{\floatsep}{6pt plus 2pt minus 2pt}
\setlength{\intextsep}{6pt plus 2pt minus 2pt}

% (Removed manual geometry configuration)
...
% (Removed duplicate early ablation table to avoid label conflict and float crowding)
...
% Ablation table: single-method vs hybrid
...
\label{tab:ablation}
\begingroup
\setlength{\tabcolsep}{3.5pt} % tighten to fit in column
\small
...
% NOTE: Replace placeholder dashes with actual baseline numbers once dense-only and sparse-only Natural Questions runs are added.
...
% Short anchoring paragraph to reduce consecutive top floats
...
% Anchoring narrative before reliability table
...
\begin{figure}[!t]
\centering
% Local caption style reduction (kept within IEEE norms: small font)
\begingroup
\captionsetup{font=small}
...
\FloatBarrier
\clearpage % start references on a fresh (final) page
\balance
